...
var allLBExpressions = map[interface{}]lbalias.CLI{
	//"SSHDAEMON":     DaemonListening{code: 7, port: 22},
	//"WEBDAEMON":     DaemonListening{code: 8, port: 80},
	//"FTPDAEMON":     DaemonListening{code: 9, port: 21},
	//"GRIDFTPDAEMON": DaemonListening{code: 11, port: 2811},
	//"COLLECTDLOAD":  ParamCheck{code: 16, command: "collectd"},
	//"LEMONLOAD":     ParamCheck{code: 17, command: "lemon"},
	"NOLOGIN":   NoLogin{code: 1},
	"TMPFULL":   TmpFull{code: 6},
	"DAEMON":    DaemonListening{code: 7},
	"AFS":       AFS{code: 10},
	"LEMON":     ParamCheck{code: 12, command: "lemon"},
...
func (lbalias *LBalias) Evaluate() {
	...
	if err != nil {
		logger.Fatal("Fatal error when attempting to open the alias configuration file [%s]", err.Error())
		panic(err)
	}
	...
	for _, line := range lines {
		...
		if len(checks) > 0 {
			...
			if b, ok := allLBExpressions[myAction].Run(line, lbalias).(bool); !b || !ok {
		...
		if len(loads) > 0 {
			...
			if cliName == "LEMON" || cliName == "COLLECTD" {
				result = int(allLBExpressions[cliName].Run(line, lbalias).(int32))
				if result == -1 {
