...
func (daemon DaemonListening) Run(args ...interface{}) interface{} {
	...
	entry, err := daemon.processDaemonMetric(args[0].(string))
	if err != nil {
		return daemon.code
	}
	...
	if (entry.port < 1) || (entry.port > 65535) {
		logger.Error("The specified port is out of range [1-65535]. Failing with code [%d]", daemon.code)
		return false
	}
...
func (daemon DaemonListening) processDaemonMetric(line string) (de daemonEntry, err error) {
	...
	if len(found) != 2 {
		// Log
		logger.Error("Incorrect syntax specified at [%s]. Please use (`check` `daemon` `<protocol><ip_level (optional)>:<port>`). Failing with code [%d]", line, daemon.code)
		return de, err
	}
	...
	if !strings.Contains(line, ":") {
		// Log
		logger.Error("Incorrect syntax specified at [%s]. Please use (`check` `daemon` `<protocol><ip_level (optional)>:<port>`). Failing with code [%s]", line, daemon.code)
		return de, err
	}
	...
	if len(rawPortNumber) != 2 {
		logger.Error("Syntax error detected when attempting to extract the port number from the metric line [%s]. The port number must be supplied using the ':' character at the end of your metric. Failing with code [%d]", line, daemon.code)
		return de, err
	}
