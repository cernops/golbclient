...
// TestCollectdFailedConfigurationFileWithKeys : integration test for all the functionality supplied by the collectdctl, fail test
func TestCollectdFailedConfigurationFileWithKeys(t *testing.T) {
        logger.SetLevel(logger.FATAL)

        cfg := utils.NewConfiguration("../test/lbclient_collectd_check_fail_with_keys.conf", "collectd_intended_fail_test_with_keys")
        err := lbalias.Evaluate(cfg)
        if err == nil {
                logger.Error("Expecting an error for the given configuration file [%s]. Failing test...", cfg.ConfigFilePath)
                t.Fail()
